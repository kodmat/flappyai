...
window = pyglet.window.Window(disp_width, disp_height, 'FLAPPY')
datas = []
outs = []
...
class pipes:
    ...
    def __init__(self):
        ...
        self.xs = np.array([[disp_width , disp_width]]).astype(np.float)
        self.ys = np.array([[-300, -300  + self.pipe_up.height + 300 ]]).astype(np.float)
...
def run(dt):
    ...
    global gravity, nsp, game_on, my_pipes, my_flappy, datas, outs, ust, a

    if game_on:

        for i in range(len(my_pipes.xs)):
            if my_pipes.xs[i][0] <= my_flappy.position.x + my_flappy.flaps[0].width and  my_flappy.position.x <= my_pipes.xs[i][0]  + my_pipes.pipe_up.width:
                if my_pipes.ys[i][0] + my_pipes.pipe_up.height <=  my_flappy.position.y and   my_flappy.position.y + my_flappy.flaps[0].height <= my_pipes.ys[i][1]:
                    pass
                else:
                    del my_flappy, my_pipes
                    my_flappy = flappy(100, disp_height / 2)
                    my_pipes = pipes()
                    my_pipes.puan = 0
                    gravity = 0
        ...
        if space and nsp:
            gravity = -12
            nsp = False
            my_flappy.rotation = - 6

        if my_pipes.xs[len(my_pipes.xs) - 1][0] <= 500 - 5 * my_pipes.puan:
            my_pipes.add_pip()

        dat = []
        for i in range(len(my_pipes.xs)):
            if my_pipes.xs[i][0] + my_pipes.pipe_up.width >= 100:
                dat = [
                    gravity,
        ...
        if dat != []:
            ...
            sonuc = model.predict(np.array(dat).reshape(-1,10))[0][0]
